...
server::server(std::string input, const unsigned short kPortNumber) : m_portNumber(kPortNumber)
{
    m_CTRKin = new solve_kinematics;

    if (!m_CTRKin->initialize(input))
    {
        ::std::cout << "No robot joint files in the specified folder" << ::std::endl;
    }
